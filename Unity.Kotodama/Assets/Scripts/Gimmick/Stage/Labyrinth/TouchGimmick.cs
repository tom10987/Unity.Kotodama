...
public class TouchGimmick : MonoBehaviour {
  ...
  void OnCollisionStay(Collision player) {
    // 灯籠の床部にプレイヤーが接触してる間
    if (player.gameObject.tag.Equals(ObjectTag.Player.ToString())) {
      _isHit = true;
    }
    else {
      _isHit = false;
  ...
  void ClearGimmick() {
    if (_isHit) {
      ...
      if (TouchController.IsTouchBegan()) {
        // カメラからタッチ位置へのレイを生成
        Ray ray_ = Camera.main.ScreenPointToRay(TouchController.GetScreenPosition());
        RaycastHit hit_ = new RaycastHit();

        // レイが当たったオブジェクトのコライダーを取得
        if (Physics.Raycast(ray_, out hit_)) {
          if (hit_.collider.gameObject.tag.Equals(ObjectTag.Gimmick.ToString())) {
            // クリアフラグをtrueにする
            if (!_isClear) {
              _isClear = true;
            }
