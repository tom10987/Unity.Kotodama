...
public class NovelSystem : SingletonBehaviour<NovelSystem> {
    ...
    void ReadingStory(string[,] cur)
    {
        ...
        if (cur[_msgCur, 0] == Command.start)
        { ReadStart(); }

        else if (cur[_msgCur, 0] == Command.end)
        { ReadEnd(); }

        else if (cur[_msgCur, 0] == Command.name)
        { ReadName(cur[_msgCur, 1]); }

        else if (cur[_msgCur, 0] == Command.image)
        { ReadImage(cur[_msgCur, 1], cur[_msgCur, 2], cur[_msgCur, 3]); }

        else if (cur[_msgCur, 0] == Command.jump)
        { ReadJump(cur[_msgCur, 1], cur[_msgCur, 2], cur[_msgCur, 3]); }
    ...
    void ReadText(string str)
    {
        ...
        { _msgRow = str.Length; _isLineEnd = true; }
        if (_msgRow == str.Length && TouchController.IsTouchEnded())
        { _msgCur++; _msgRow = 0; _isLineEnd = false; }
    }
